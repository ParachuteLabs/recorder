...
extern bool is_recording; // From button.c
...
void set_led_state()
{
    // Charging indicator - GREEN blinks
    if(usb_charge)
    {
    ...
    // If device is off, no other LEDs
    if(is_off)
    {
        set_led_red(false);
        set_led_blue(false);
    ...
    // Recording state takes priority - RED LED
    if (is_recording)
    {
        // Recording active - red LED on, blue off
        set_led_red(true);
        set_led_blue(false);
        return;
    }
    
    // Connected state - BLUE (only when not recording)
    if (is_connected)
    {
        set_led_blue(true);
        set_led_red(false);
    ...
    // Not connected and not recording - no LEDs
    if (!is_connected)
    {
        set_led_blue(false);
        set_led_red(false);
